...
def train_model(
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if val_metric < best_val_loss:
            ...
            # Save the model and embeddings only when there's an improvement in validation loss
            if save_best:
                save_model(model, user_embeddings=None, item_embeddings=None, output_dir=checkpoint_dir, model_name=MODEL_NAME)
                epoch_log += f", Best Val Model (Val BPR-Loss = {best_val_loss:.4f}), Model Saved"
                logger.info(f"📌 Saved new best model (Val BPR-Loss={best_val_loss:.4f})")
        else:
            epochs_no_improve += 1
